...
plot_popdyn <- function(sces=sces,
   ...
   lst_popdyn <- lapply(sces, function(sce) {
      res <- get(paste("lst_popdyn_", sce, sep = ''), env = .GlobalEnv)
      ...
      namesimu1          <- c(names(unlist(lapply(res, function(x) nrow(x) == expected_nb_rows)))[idx])
      res[namesimu1]
   })
