...
if(TRUE){
 ...
 if(.Platform$OS.type == "windows") {
  ...
  general$main.path.igraph  <- file.path("D:","FBA",paste("DISPLACE_input_",general$case_study, sep=""), "graphsspe")
  general$main.path.param   <- file.path("D:","FBA",paste("DISPLACE_input_gis_",general$case_study, sep=""))
  general$main.path.ibm     <- file.path("D:","FBA", paste("DISPLACE_input_", general$case_study, sep='')) 
 }

 if(general$case_study=="Baltic21"){
    general$igraph            <- 100
    general$a.year            <- "2019"
    general$a.country         <- c("DEU", "DNK", "EST", "FIN", "LTU", "LVA", "POL", "SWE")
    general$nbpops            <- 37  
    general$nbszgroup         <- 14
    general$namefolderinput   <- "Baltic21"
    ...
    general$namefolderoutput   <- c(
                                 ...
                                 "sceseverelowfmsy",
                                 "sceseverepshoke",
                                 "sceseverepshokelowfmsy",
                                 "scespatial",
                                 "scespatiallowfmsy",                                 
                                 "scebaselineeffmax",                                 
                                 "scebaselowfmsyeffmax"
                                 )
       general$namesimu           <- list(
                                 "scebaseline"=   paste("simu", c(1:10), sep=''),
                                 "scebaselowfmsy"=paste("simu", c(1:10), sep=''),
                                 "scemild"=paste("simu", c(1:10), sep=''),
                                 "scemildlowfmsy"=paste("simu", c(1:10), sep=''),
                                 "scesevere"=paste("simu", c(1:10), sep=''),
                                 "sceseverelowfmsy"=paste("simu", c(1:10), sep=''),
                                 "sceseverepshoke"=paste("simu", c(1:10), sep=''),
                                 "sceseverepshokelowfmsy"=paste("simu", c(1:10), sep=''),
                                 "scespatial"=paste("simu", c(1:10), sep=''),
                                 "scespatiallowfmsy"=paste("simu", c(1:10), sep=''),     
                                 "scebaselineeffmax"=paste("simu", c(1:10), sep=''),     
                                 "scebaselowfmsyeffmax"=paste("simu", c(1:10), sep='')     
                                 ) 
   the_scenarios1 <-  c(  "FMSY", 
                          "FMSY Low", 
                          "FMSY + RCP4.5",
                          "FMSY Low + RCP4.5",
                          "FMSY + RCP8.5",
                          "FMSY Low + RCP8.5",
                          "FMSY + RCP8.5 + Shoke",
                          "FMSY Low + RCP8.5 + Shoke",
                          "FMSY + Range contraction",
                          "FMSY Low + Range contraction",
                          "FMSY + effort max",
                          "FMSY Low + effort max"
                          )
       ...
       general$igraph   <- c(
                                 ...
                                 "scemildlowfmsy"=100,
                                 "scesevere"=100,
                                 "sceseverelowfmsy"=100,
                                 "sceseverepshoke"=100,
                                 "sceseverepshokelowfmsy"=100,
                                 "scespatial"=100,
                                 "scespatiallowfmsy"=100,  
                                 "scebaselineeffmax"=100,                                 
                                 "scebaselowfmsyeffmax"=100
                                 )
 ...
 # add the dominant metier for the trips
 loglike$dominant_metier <- gsub(")","", gsub("M\\(", "", sapply(strsplit(as.character(loglike$freq_metiers), split="_"), function(x)x[1])))
 ...
 coord <- read.table(file=file.path(general$main.path.ibm, "graphsspe", 
             paste("coord", general$igraph[sce], ".dat", sep=""))) # build from the c++ gui
 coord <- as.matrix(as.vector(coord))
 coord <- matrix(coord, ncol=3)
 colnames(coord) <- c('x', 'y', 'harb')
 ...
 aggregateLoglike <- function(loglike, agg_by=c("year.month"), what="weight"){
    ...
    # AGGREGATE PER SPECIES -----> MEAN    (CAUTION: AVERAGE OVER VESSELS AND PER MONTH/PERIOD.....)
    idx.col.e  <- grep('effort', nm)
    idx.col.b  <- grep('bwtrip', nm)
    ...
    idx.col.GVA <- grep('GVA', nm, fixed = TRUE)
    idx.col.GVAPerRevenue <- grep('GVAPerRevenue', nm, fixed = TRUE)
    idx.col.LabourSurplus <- grep('LabourSurplus', nm, fixed = TRUE)
    idx.col.GrossProfit <- grep('GrossProfit', nm, fixed = TRUE)
    idx.col.NetProfit <- grep('NetProfit', nm, fixed = TRUE)
    idx.col.NetProfitMargin <- grep('NetProfitMargin', nm, fixed = TRUE)
    idx.col.GVAPerFTE <- grep('GVAPerFTE', nm, fixed = TRUE)
    idx.col.RoFTA <- grep('RoFTA', nm, fixed = TRUE)
    idx.col.BER <- grep('BER', nm, fixed = TRUE)
    idx.col.CRBER <- grep('CRBER', nm, fixed = TRUE)
    idx.col.NetPresentValue <- grep('NetPresentValue', nm, fixed = TRUE)
    #idx.col.disc_rate  <- which(nm %in% paste('disc_rate_', explicit_pops, sep=''))
    idx.col <- c(idx.col.e, idx.col.b, idx.col.t,  idx.col.v, idx.col.v2, 
     #idx.col.disc_rate,
      idx.col.rsa,
     idx.col.GVA, idx.col.GVAPerRevenue, idx.col.LabourSurplus,idx.col.GrossProfit, idx.col.NetProfit, idx.col.NetProfitMargin,
      idx.col.GVAPerFTE, idx.col.RoFTA, idx.col.BER, idx.col.CRBER, idx.col.NetPresentValue)
    ...
    loglike.agg <- loglike.agg [,!(grepl('.1', colnames(loglike.agg )) && !grepl('.pop', colnames(loglike.agg )))]  # filter the redundant columns out (arise from partial grep)
 ...
 loglike.agg.met <-  aggregateLoglike(loglike, agg_by=c( "dominant_metier", "year.month"))
...
## plot landings per species
 ...
 lst_loglike_agg_met[[sim]] <- loglike.agg.met
 ...
 assign(paste("lst_loglike_agg_",what,"_met_", sce, sep=''), lst_loglike_agg_met) 
 ...
 save(list=c(paste("lst_loglike_agg_",what,"_all_", sce, sep=''),
       ...
       if(!is.null(selected_vessels_set3)){ paste("lst_loglike_agg_",what,"_selected_set3_", sce, sep='')},
          ...
          paste("lst_loglike_agg_",what,"_met_", sce, sep='')
...
return()
 ...
 ## CALLS-----------------------------------------------------------------------
  # first of all, figure out the list of vessels we want to keep for _selected...
   ...
   if(general$namefolderinput=="Baltic21" && .Platform$OS.type == "unix"){
     
     implicit_pops <- c(0, 5, 6, 7, 8, 9, 10, 15, 16, 17, 18, 19, 20, 21, 24, 25, 26, 28, 29, 30, 32, 33, 34, 35, 36)
     ...
     ##
     ##
     ## TODO: ADAPT TO GROUP VESSELS PER RELEVANT FISHERIES, e.g. looking at gear and main target species in the a loglike logbooks......
     ##
     ##
     ##
     dominant_metier <- gsub(")","", gsub("M\\(", "", sapply(strsplit(as.character(loglike$V48), split="_"), function(x)x[1])))
     loglike$dominant_metier <- dominant_metier
     vid_met <- loglike[!duplicated(loglike$V7, loglike$dominant_metier), c('V7','dominant_metier')]
     vids_activebottomgears <- as.character(unique(vid_met[vid_met$dominant_metier %in% c(10, 12,14,17,18), 1]))     # see metiersspe\metier_names.dat
     vids_activepelagicgears <- as.character(unique(vid_met[vid_met$dominant_metier %in% c(11,13,15), 1]))     # see metiersspe\metier_names.dat
     vids_passivegears <- as.character(unique(vid_met[vid_met$dominant_metier %in% c(0:9, 16), 1]))     # see metiersspe\metier_names.dat
     # caution: a vid can be at the meantime in different metier categories...
...
if(.Platform$OS.type != "unix"){
 ...
 if(general$case_study=="Baltic21"){ 
   source(file.path("D:","FBA","DISPLACE_input_gis_Baltic21","DISPLACE_R_outputs_ForBaltic21","loadAggLoglikeFiles.R"))
...
if(general$namefolderinput=="Baltic21" && TRUE){

 
   selected <- "_selected_set1_"  # all vid    
   selected <- "_selected_set2_"  # passive only   
   selected <- "_selected_set3_"  # trawlers   

sets <- c("_selected_set1_", "_selected_set2_", "_selected_set3_")
for (selected in sets){

if(selected=="_selected_set1_")
 {
 a_title <- "All Gears"
 }
if(selected=="_selected_set2_")
 {
 a_title <- "Passive Gears"
 }
if(selected=="_selected_set3_")
 {
 a_title <- "Bottom-Contacting Gears"
 }
 ...
 baseline <- outcomes[outcomes$scenario == "scespatial",]  # init
 ...
 if(select_some){
 if(selected== "_selected_set2_"){
      selected_variables <- c("feffort", "seffort", "sweptarea", "nbtrip", "av_trip_duration", "fishing_based_cpue", #"fishing_based_cpue_explicit",
                                       "totland", #"totland_explicit", 
                                       "totland_2", "totland_3", "totland_11", "totland_12", "totland_22", "totland_23",  "totland_31",
                                          "npv", "av_vpuf_month", "hoover")
       outcomes           <- outcomes[outcomes$variable %in% selected_variables,]
 
       outcomes$variable <- factor(outcomes$variable)
       outcomes$variable <- factor(outcomes$variable, levels=selected_variables, labels= c( "F. effort", "S. effort", "Swept Area",  "Nb. of trips", "Trip duration",  "CPUE at fishing",
                                                                                       "Tot landings",
                                                                                       "Landings COD.2224", 
                                                                                       "Landings COD.2532",
                                                                                       "Landings HER.3a22",
 ...
 # a better plot
 namefile       <- paste(paste("indicators_boxplot_persce_",selected, "_revised", sep=""))
 ...
 print(
       p   + ggtitle(a_title) + facet_wrap( ~ scenario, ncol=2, scales="free_y")    + theme_bw()+
   ...
   for(vid in unique(lst_loglike_agg_vid[[1]]$VE_REF)){
        ...
        cat (paste(vid, "\n"))
          ...
          if(length(grep("SWN", vid))!=0) {
            ...
            if(per_vessel &&  sce=="scebaseline") {  # PER VESSEL
...
compare_sim_sim_landings <- function (lst_loglike_agg_1, lst_loglike_agg_2, ...,  plot_obs=TRUE, 
    ...
    tstep                               <- paste(rep(2019:2029, each=12), c("01","02","03","04","05","06","07","08","09","10","11","12"), sep=".")
...
if(general$namefolderinput=="Baltic21"){
  ...
  #!#!#!#!#!#!#!#!#!#
    ...
    for(vid in unique(lst_loglike_w_agg_vid_1[[1]]$VE_REF)){
        ...
        cat (paste(vid, "...", count/length(unique(lst_loglike_w_agg_vid_1[[1]]$VE_REF)) *100,  "% \n"))  
          ...
          #if(length(grep("DEN", vid))!=0 || length(grep("DEN", vid))!=0 || length(grep("IRL", vid))!=0 || length(grep("GBR", vid))!=0 || length(grep("NLD", vid))!=0 || length(grep("FRA", vid))!=0 ){
            per_vessel <- TRUE
            per_pop    <- FALSE # per pop per vessel: to fix: imcompatibble with per_vessel
