...
# Net profit Margin = Net Profit / revenue. NOT GOOD
check = EconomicsPertrip %>%
    arrange(VE_REF,TStepDep,TStep) %>%
    group_by(VE_REF) %>%
    ...
    #mutate(NetProfitMarginEstim = replace(NetProfitMarginEstim,cumRevenue<=0,0)) %>%
    mutate(SanityCheck = abs(NetProfitMarginEstim - NetProfitMargin))%>%
    #filter(SanityCheck<100000) %>% 
    ggplot(aes(x=TStep,y=SanityCheck,group=VE_REF))+
      geom_line(alpha=0.1)+
      theme_minimal()
